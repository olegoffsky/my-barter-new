...
class RobokassaPayment {
  ...
  public static function button($amount = '0.00', $description = '', $itemnumber = '', $extra_array = null) {
    ...
    $sign  = md5($shop_id.':'.round($amount, 2).':'.$order_id.':'.$password_1);
    $url ="https://auth.robokassa.ru/Merchant/Index.aspx?MerchantLogin=".$shop_id."&IsTest=0&OutSum=".round($amount, 2)."&InvId=".$order_id."&Description=".$description."&SignatureValue=".$sign;
