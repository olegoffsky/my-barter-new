          ...
          foreach($params as $name => $value)
{
  $values .= $value;
          ...
          $signature = base64_encode(pack("H*", md5($values . $mrhPass1)));
 
          if ($signature == Params::getParam("WMI_SIGNATURE")) {
            if (strtoupper(Params::getParam("WMI_ORDER_STATE")) == "ACCEPTED") {
    
    $product_type = Params::getParam("shpType");
	$invId = Params::getParam("shpBype");
	$outSumm = Params::getParam("WMI_PAYMENT_AMOUNT");
    $currency = osc_get_preference('currency', 'rupayments');
    $sTransactionId = Params::getParam("WMI_PAYMENT_NO");	
 // это новое
	...
	if ( ModelRUpayments::newInstance()->getIsPublishPaymentNeeded ( $iItemId ) ) {
	       ...
	       if ( isset ( ModelRUpayments::newInstance()->lastId ) ) $payment_id = ModelRUpayments::newInstance()->lastId;
            ...
            if ($product_type == '101') {
                ...
                ModelRUpayments::newInstance()->addWallet ( $iUser, Params::getParam('WMI_PAYMENT_AMOUNT')!=''?Params::getParam('WMI_PAYMENT_AMOUNT') + $bonus_amount:Params::getParam('WMI_PAYMENT_AMOUNT') + $bonus_amount);
            }
				print_answer("Ok", "order #" . $sTransactionId . " paid!");
			}
