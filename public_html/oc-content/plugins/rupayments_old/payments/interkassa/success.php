 ...
 */
    ...
    if ( is_array ( $aDataInner ) ) { 
        ...
        if ( count ( $aDataInner ) < 3 ) {
            osc_add_flash_info_message(__('Time of session of your Interkassa has expired, please try again', 'rupayments'));
            rupayments_js_redirect_to(osc_base_url());
        }
  
        if ( $aData = ModelRUpayments::newInstance()->getPaymentByCode ( $transaction_id, "Interkassa" ) ) {
