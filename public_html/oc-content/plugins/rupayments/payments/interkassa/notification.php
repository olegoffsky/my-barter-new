 ...
 */
 define('ABS_PATH', dirname(dirname(dirname(dirname(dirname(dirname(__FILE__)))))) . '/');
	...
	$ik_payment_amount = Params::getParam('ik_am');
	$sTransactionId = Params::getParam('ik_pm_no');
	$dataSet = $_REQUEST;
    $sNotificationSecret = osc_get_preference('interkassa_secret', 'rupayments');
unset($dataSet['ik_sign']);
ksort($dataSet, SORT_STRING); 
array_push($dataSet, $sNotificationSecret);
$signString = implode(':', $dataSet); 
$sign = base64_encode(md5($signString, true));
	    $bStatus = false;
    	$status_data = Params::getParam('ik_sign');
    ...
    if ( $bStatus ) {
        ...
        if ( !ModelRUpayments::newInstance()->isTransactionMade ( $sTransactionId ) ) {
            ...
            */
            ...
            $payment_id = ModelRUpayments::newInstance()->saveLog(
                                                                $sConcept, //concept
                                                                $sTransactionId,
                                                                Params::getParam('ik_am') !=''?Params::getParam('ik_am'):Params::getParam('ik_am'), //ik_am
                                                                $sCurrency, //currency
     ...
     ModelRUpayments::newInstance()->setTopItem ( $iItemId );  
                ...
                ModelRUpayments::newInstance()->addWallet ( $iUser, Params::getParam('ik_am')!=''?Params::getParam('ik_am') + $bonus_amount:Params::getParam('ik_am') + $bonus_amount);
            }
        }
    }
