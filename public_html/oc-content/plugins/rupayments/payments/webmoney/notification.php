 ...
 */
 define('ABS_PATH', dirname(dirname(dirname(dirname(dirname(dirname(__FILE__)))))) . '/');
	...
	$sTransactionId = Params::getParam('FIELD_1');
	$status_data = Params::getParam('LMI_HASH');
	$mrhLogin = osc_get_preference('webmoney_id', 'rupayments');
    $sNotificationSecret = osc_get_preference('webmoney_secret', 'rupayments');
	$my_hash = strtoupper(hash('sha256', $mrhLogin.Params::getParam('LMI_PAYMENT_AMOUNT').Params::getParam('LMI_PAYMENT_NO').Params::getParam('LMI_MODE').Params::getParam('LMI_SYS_INVS_NO').Params::getParam('LMI_SYS_TRANS_NO').Params::getParam('LMI_SYS_TRANS_DATE').$sNotificationSecret.Params::getParam('LMI_PAYER_PURSE').Params::getParam('LMI_PAYER_WM')));
    $bStatus = false;
    if ( $status_data == $my_hash ) {
    ...
    if ( $bStatus ) {
        ...
        if ( !ModelRUpayments::newInstance()->isTransactionMade ( $sTransactionId ) ) {
            ...
            */
            ...
            $payment_id = ModelRUpayments::newInstance()->saveLog(
                                                                $sConcept, //concept
                                                                $sTransactionId,
                                                                Params::getParam('LMI_PAYMENT_AMOUNT') !=''?Params::getParam('LMI_PAYMENT_AMOUNT'):Params::getParam('LMI_PAYMENT_AMOUNT'), //ik_am
                                                                $sCurrency, //currency
     ...
     ModelRUpayments::newInstance()->setTopItem ( $iItemId );  
                ...
                ModelRUpayments::newInstance()->addWallet ( $iUser, Params::getParam('LMI_PAYMENT_AMOUNT')!=''?Params::getParam('LMI_PAYMENT_AMOUNT') + $bonus_amount:Params::getParam('LMI_PAYMENT_AMOUNT') + $bonus_amount);
            }
        }
    }
