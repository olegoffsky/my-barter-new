...
if (Params::getParam('MERCHANT_ORDER_ID') !== '' && Params::getParam('SIGN') !== '')
{	
	$ik_payment_amount = Params::getParam('AMOUNT');
	$sTransactionId = Params::getParam('MERCHANT_ORDER_ID');
    $merchant_id = osc_get_preference('ik_co_id_free', 'rupayments');
    $merchant_secret = osc_get_preference('freekassa_secret2', 'rupayments');
    $sign_hash = md5($merchant_id.':'.$ik_payment_amount.':'.$merchant_secret.':'.$sTransactionId);
    $err = false;
	$message = '';



	if (Params::getParam('SIGN') != $sign_hash)
		{
    ...
    if ( !$err ) {
        ...
        if ( !ModelRUpayments::newInstance()->isTransactionMade ( $sTransactionId ) ) {
            ...
            */
            ...
            $payment_id = ModelRUpayments::newInstance()->saveLog(
                                                                $sConcept, //concept
                                                                $sTransactionId,
                                                                Params::getParam('AMOUNT') !=''?Params::getParam('AMOUNT'):Params::getParam('AMOUNT'), //ik_am
                                                                $sCurrency, //currency
     ...
     ModelRUpayments::newInstance()->setTopItem ( $iItemId );  
                ...
                ModelRUpayments::newInstance()->addWallet ( $iUser, Params::getParam('AMOUNT')!=''?Params::getParam('AMOUNT') + $bonus_amount:Params::getParam('AMOUNT') + $bonus_amount);
            }
        }
		print "Yes";
            exit();
