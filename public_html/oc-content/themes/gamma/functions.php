<?php
define('GAMMA_THEME_VERSION', '1.0.2');
...
function gam_is_lazy() {
  if(gam_param('lazy_load') == 1 && osc_get_preference('force_aspect_image', 'osclass') == 0) {
    return true;
  }
...
function gam_profile_picture($user_id = '', $size = 'small') {
...
function gam_def_location() {
  ...
  foreach($countries as $c) {
    ...
    if($counter == $limit*count($countries)) {
      $html .= '<div class="option service empty-pick default" data-country="" data-region="" data-city="" data-code="" id=""><em>' . sprintf(__('... and %d more %s, enter your %s name to refine results', 'gamma'), $limit, $type_name, $type_name) . '</em></div>';
    }
  }
...
function gam_locbox_short($country = '', $region = '', $city = '', $level = 'all') {
  ...
  if(count($countries) > 1 && ($level == 'all' || $level == 'country')) {
    ...
    foreach($countries as $c) {
      $html .= '<div class="elem country ' . (strtoupper($c['pk_c_code']) == strtoupper($country) ? 'active' : '') . '" data-country="' . $c['pk_c_code'] . '" data-region="" data-city="" style="' . (gam_param('loc_one_row') == 1 ? 'width:' . ($box_width + 1) . 'px;'  : '') . '"><img src="' . osc_current_web_theme_url() . 'images/country_flags/large/' . strtolower($c['pk_c_code']) . '.png"/><strong>' . $c['s_name'] . '</strong></div>';
    }
  ...
  if(count($categories) > 0) {
    ...
    foreach($categories as $c) {
      ...
      if($c['fk_i_parent_id'] <= 0) {
        $html .= '<div class="img">' . gam_get_cat_icon( $c['pk_i_id'] ) . '</div> <strong>' . $c['s_name'] . '</strong>';
      } else {
...
function gam_is_rtl() {
  ...
  if(in_array($current_lang, gam_rtl_languages())) {
    return true;
  } else {
    return false;
  }
...
function gam_user_menu() {
  ...
  echo '<div class="user-menu-header">' . __('My listings', 'gamma') . '</div>';
  ...
  echo '<div class="user-menu-header">' . __('My account', 'gamma') . '</div>';
  echo '<ul class="user_menu">';
...
function gam_location_format($country = null, $region = null, $city = null) { 
  if($country <> '') {
    ...
    if($region <> '') {
      if($city <> '') {
        return $city . ' ' . __('in', 'gamma') . ' ' . $region . ($country_full['s_name'] <> '' ? ' (' . $country_full['s_name'] . ')' : '');
      } else {
...
if( !function_exists('logo_header') ) {
  function logo_header() {
    ...
    if(file_exists(WebThemes::newInstance()->getCurrentThemePath() . 'images/logo.jpg')) {
      return $html;
    } else if( gam_param('default_logo') && (file_exists( WebThemes::newInstance()->getCurrentThemePath() . 'images/default-logo.jpg')) ) {
