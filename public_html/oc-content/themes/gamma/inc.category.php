...
<?php if(osc_is_home_page() || (osc_is_search_page() && $search_cat_id <= 0)) { ?>
  <?php if(osc_is_search_page() && $search_cat_id <= 0) { ?><div class="search-top-cat"><?php } ?>

  <div id="home-cat2">
    <div class="inside">
      <h2><?php _e('Discover our categories', 'gamma'); ?></h2>
            ...
            <?php while(osc_has_categories()) { ?>
              ...
              <a href="<?php echo osc_search_url($search_params); ?>">
                <div>
                  ...
                  <?php } else { ?>
                    <img src="<?php echo gam_get_cat_image(osc_category_id()); ?>" alt="<?php echo osc_esc_html(osc_category_name()); ?>" />
                  <?php } ?>
                </div>
  ...
  <?php if(osc_is_search_page() && $search_cat_id <= 0) { ?></div><?php } ?>
<?php } ?>
    ...
    <div id="sub-cat">
      <div class="inside">
        <div class="navi">
          ...
          <a href="<?php echo osc_search_url($search_params); ?>"><?php _e('All categories', 'gamma'); ?></a>
          <svg style="transform:rotate(-90deg);" viewBox="0 0 32 32" width="20px" height="20px"><defs><path id="mbIconAngle" d="M12.147 25.2c-.462 0-.926-.185-1.285-.556L.57 14.024A2.05 2.05 0 010 12.586c0-.543.206-1.061.571-1.436L10.864.553a1.765 1.765 0 012.62.06c.71.795.683 2.057-.055 2.817l-8.9 9.16 8.902 9.183c.738.76.761 2.024.052 2.815a1.78 1.78 0 01-1.336.612"></path></defs><use fill="currentColor" transform="matrix(0 -1 -1 0 29 24)" xlink:href="#mbIconAngle" fill-rule="evenodd"></use></svg>
          ...
          <div class="list nice-scroll">
            ...
            <?php if(count($subcats) > 14) { ?>
              <div class="link show-all">
                <a href="#">
                  <span class="name"><?php echo __('Show all', 'gamma'); ?></span>
