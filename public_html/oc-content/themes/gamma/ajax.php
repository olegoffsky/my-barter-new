...
if(@$_GET['ajaxLoc2'] == '1') {
  ...
  echo gam_locbox_short($_GET['country'], $_GET['region'], $_GET['city'], $level);
  exit;
}
...
if(@$_GET['ajaxItem'] == '1' && @$_GET['sPattern'] <> '') {
  ...
  foreach( $prepare as $i => $p ) {
    ...
    $prepare[$i]['i_price'] = gam_ajax_item_format_price($prepare[$i]['i_price'], $prepare[$i]['fk_c_currency_code']);
...
if(@$_GET['ajaxPhoneClick'] == '1' && @$_GET['itemId'] > 0) {
  gam_increase_clicks($_GET['itemId'], $_GET['itemUserId']);
  echo 1;
  exit;
}
