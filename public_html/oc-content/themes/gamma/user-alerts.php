...
<body id="body-user-alerts" class="body-ua">
  ...
  <?php
    ...
    while(osc_has_alerts()) { 
      ...
      if($loc == '' && $cat == '') {
        $name = sprintf(__('Alert #%s', 'gamma'), $alert['pk_i_id']);
      } elseif ($loc == '' && $cat <> '') {
        $name = sprintf(__('%s results', 'gamma'), $cat);
      } elseif ($loc <> '' && $cat == '') {
        $name = sprintf(__('%s results', 'gamma'), $loc);
      } else {
        $name = sprintf(__('%s - %s', 'gamma'), $cat, $loc);
      }
  ...
  <div class="inside user_account">
    <div class="usr-menu alerts-menu">
      <?php if(osc_count_alerts() <= 0) { ?>
        <div class="usr-empty"><?php _e('No alerts found', 'gamma'); ?></div>
      <?php } else { ?>
        <?php $c = 0; ?>
        <?php foreach($alerts as $a) { ?>
          <div data-id="<?php echo $a['pk_i_id']; ?>" <?php if($c == 0) { ?>class="active"<?php } ?>>
            <strong><?php echo $a['name']; ?></strong>
            <span><?php echo sprintf(__('%s listings', 'gamma'), $a['count']); ?></span>

            <a href="<?php echo $a['unsubscribe_url']; ?>" class="del" title="<?php echo osc_esc_html(__('Unsubscribe', 'gamma')); ?>" onclick="javascript:return confirm('<?php echo osc_esc_js(__('This action can\'t be undone. Are you sure you want to continue?', 'gamma')); ?>');"><i class="fas fa-trash"></i></a>
          </div>
    ...
    <div id="main" class="alerts">
      <div class="inner-box">
        ...
        <div class="inside">
            ...
            <?php foreach($alerts as $a) { ?>
              <div class="alert-box" data-id="<?php echo $a['pk_i_id']; ?>" <?php if($c == 0){ ?>style="display:block;"<?php } ?>>
                ...
                <?php if(osc_count_items() > 0) { ?>
                  ...
                  <?php } ?>
                <?php } else { ?>
                  <div class="ua-items-empty"><img src="<?php echo osc_current_web_theme_url('images/ua-empty.jpg'); ?>"/> <span><?php _e('No listings match to this alert', 'gamma'); ?></span></div>
                <?php } ?>
              </div>
          ...
          <?php } else { ?>
            <div class="ua-items-empty"><img src="<?php echo osc_current_web_theme_url('images/ua-empty.jpg'); ?>"/> <span><?php _e('You have no active alerts', 'gamma'); ?></span></div>
          <?php  } ?>
        </div>
      </div>
    </div>
